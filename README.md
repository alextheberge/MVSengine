...
- Java and C# public API extraction tracks public types and public methods while stripping leading annotations or attributes out of the stored signature; member methods are owner-qualified as `java:method public String AuthApi.login(...)` and `csharp:method public static string AuthApi.Login(...)`
- Kotlin public API extraction tracks public or default-visible `class`, `interface`, `object`, and `fun` declarations, preserving modifiers such as `data` and `suspend` while skipping `private`, `protected`, and `internal`; member functions are stored as owner-qualified signatures such as `kotlin:fun AuthApi.login(...)`
