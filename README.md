...
Member signatures inside class-like scopes are now owner-qualified so inventories stay stable when multiple types expose the same method or property names. Typical forms include `python:def Worker.run_job(...)`, `ruby:def Demo::AuthApi#login(...)`, `java:field public String AuthApi.status`, `csharp:property public string AuthApi.DisplayName { get }`, `kotlin:val AuthApi.token: String`, `php:public readonly string AuthApi.$token`, and `swift:public func AuthApi.login(...)`.
...
- Python public API extraction tracks public `class` and non-underscore `def` declarations, public `type` aliases, module-level constants such as `API_VERSION` or `__all__`, and public class-level constants such as `Worker.STATUS`, without promoting nested local helpers or private class bodies into the API inventory; class methods are stored as owner-qualified signatures such as `python:def Worker.run_job(...)`
- Java public API extraction tracks public types, public fields, interface constants, and public or interface methods while stripping leading annotations out of the stored signature; member signatures are owner-qualified as `java:field public String AuthApi.status`, `java:const public static final String AuthApi.Contract.STATE`, and `java:method public String AuthApi.login(...)`
- C# public API extraction tracks public types, public fields, public constants, public properties, and public or interface methods while stripping leading attributes out of the stored signature; member signatures are owner-qualified as `csharp:field public static readonly string AuthApi.Version`, `csharp:const public string AuthApi.STATUS_READY`, `csharp:property public string AuthApi.DisplayName { get }`, and `csharp:method public static string AuthApi.Login(...)`
- Kotlin public API extraction tracks public or default-visible `class`, `interface`, `object`, `fun`, `val`, `var`, and top-level `const val` declarations, preserving modifiers such as `data` and `suspend` while skipping `private`, `protected`, and `internal`; member functions and properties are stored as owner-qualified signatures such as `kotlin:fun AuthApi.login(...)` and `kotlin:val AuthApi.token: String`
...
- Lua public API extraction tracks global `function` declarations and returned module-table exports such as `Api.connect = function(...) end`, `function Api:refresh(...)`, and named fields from returned tables, while `--` and long-bracket comments remain decorator-aware
