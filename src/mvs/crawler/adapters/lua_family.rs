...
impl LuaDialect {
    ...
    fn supports_export_types(self) -> bool {
    ...
    for child in named_children(node) {
        match child.kind() {
            ...
            "function_declaration" => {
                ...
                {
                    signatures.push(format!("{}:{signature}", dialect.label()));
                }
            }
            "type_definition" if dialect.supports_export_types() => {
...
fn collect_assignment_exports(
    ...
    for (target, value) in extract_assignment_pairs(assignment) {
        ...
        signatures.push(format!("{}:field {member_name}", dialect.label()));
    }
}
...
fn collect_table_constructor_exports(
    ...
    for field in named_children(table_constructor) {
        ...
        signatures.push(format!("{}:field {target}", dialect.label()));
    }
}
